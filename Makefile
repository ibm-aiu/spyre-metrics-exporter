...
IMAGE_NAME 			:= $(REGISTRY)/spyre-metrics-exporter
IMAGE 				?= $(IMAGE_NAME):$(IMAGE_TAG)

MOCK_IMAGE_NAME		:= $(REGISTRY)/spyre-metrics-mock-user
...
clean-exit: ## Cleanup with exit 1
	@$(DOCKER) ps
	@$(DOCKER) logs single-spyre-exporter || true
	@$(DOCKER) logs multi-spyre-exporter || true
	@$(MAKE) exporter-cleanup
	@exit 1
